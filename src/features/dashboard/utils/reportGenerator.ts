...
import { Employee, AttendanceLog } from '@/shared/types';
...
export const generateAttendanceReport = (
  ...
  const tableData = employees.map(emp => {
    const empLogs = logs.filter(log => 
      ...
      format(new Date(log.timestamp), 'MMMM yyyy') === monthName
    );
    
    return [
      emp.name,
      emp.email,
      empLogs.filter(l => l.type === 'check-in').length,
      empLogs.filter(l => l.type === 'check-out').length,
      empLogs.length
    ];
  });
